...
local function filter_complex_blocks(body)
  ...
  for line in body:gmatch('[^\r\n]+') do
    ...
    then
      ...
      -- avoid redefine
      line = (line:gsub('extern struct', 'struct'))
...
local function preprocess(path)
  ...
  -- stylua: ignore
  if os.getenv('NVIM_TEST_PRINT_CDEF') == '1' then for lnum, line in ipairs(new_lines) do pp(lnum, line) end end
